...
from hecos.hpm.reminder import store, scheduler, parser
...
class ReminderTools:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"[REMINDER] Failed to auto-start scheduler: {e}")
        ...
        # Autonomous config is handled via reminder_config module.
    ...
    def snooze_reminder(self, reminder_id: str, minutes: int = 15) -> str:
        ...
        self.stop_audio(reminder_id=rid)  # Stop currently ringing alarm
        return translator.t("ext_reminder_snoozed", title=reminder["title"], time=time_str)

    def stop_audio(self, reminder_id: str = None) -> str:
        """
        ...
        try:
            ...
            if sm is not None:
                sm.add_event("reminder_stop", {"id": reminder_id or "all"})
        except Exception as e:
            logger.debug("REMINDER", f"Could not broadcast reminder_stop: {e}")
