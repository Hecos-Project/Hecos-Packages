...
def save_export_to_disk(list_id):
    ...
    import os
    from hecos.hpm.lists import store as _store
        ...
        if lst:
            for item in pl.get("items", []):
                ...
                if item.get("status") == "done":
                    from hecos.hpm.lists import store as _s
...
def restore_lists():
    ...
    from hecos.hpm.lists import store
    data_req = request.get_json(force=True) or {}
    lists_data = data_req.get("data", [])
    # Support legacy "lists" key
    if not lists_data and "lists" in data_req:
