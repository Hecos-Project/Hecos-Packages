...
# We use a unique name 'hecos_editor' to avoid internal Flask collisions
editor_bp = Blueprint(
    "hecos_editor", 
    __name__, 
...
# Map extension â†’ Monaco language ID
LANG_MAP = {
...
def _safe_path(root: str, rel_path: str) -> str | None:
    ...
    """
    import sys
    # If the provided path is already absolute (e.g., "C:\Hecos-Core\main.py")
...
def init_routes(app):
    ...
    if "hecos_editor" not in app.blueprints:
        app.register_blueprint(editor_bp)
        logger.info("[Editor] Blueprint 'hecos_editor' registered.")
...
def drive_editor_save():
    ...
    try:
        ...
        temp_path = target + ".hecos_editor_tmp"
        ...
        if os.path.exists(target + ".hecos_editor_tmp"):
            os.remove(target + ".hecos_editor_tmp")
        return jsonify({"ok": False, "error": str(e)}), 500
