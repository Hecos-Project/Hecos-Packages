...
_main_path = os.path.join(_base_dir, "main.py")
_spec = importlib.util.spec_from_file_location("hecos_drive_main", _main_path)
hecos_drive_main = importlib.util.module_from_spec(_spec)
sys.modules["hecos_drive_main"] = hecos_drive_main
_spec.loader.exec_module(hecos_drive_main)
...
def _get_quick_links(root_dir: str) -> list:
    """
    Scans well-known Hecos directories and returns a list of groups,
    ...
    groups.append({
        ...
        "title": "🚀 Shortcuts",
        "items": [
            {"name": "📁 Media Folder", "path": os.path.normpath(os.path.join(root_dir, "hecos/media")).replace("\\", "/")}
        ]
    })
...
def init_drive_routes(app, logger_instance=None):
    ...
    # ─── Load Extensions ────────────────────────────────────────────────────────
    try:
        from hecos.core.system.extension_loader import load_extension_routes, discover_extensions
        ...
        try:
            with open("C:\\Hecos-Core\\drive_editor_crash.txt", "w") as f:
    ...
    # ─── Register Drive Blueprint ──────────────────────────────────────────────
    if "hecos_drive" not in app.blueprints:
        app.register_blueprint(drive_bp)
        _log.info(f"[Drive] Blueprint 'hecos_drive' registered (Prefix: {drive_bp.url_prefix or '/'})")
    else:
        _log.debug("[Drive] Blueprint 'hecos_drive' already registered.")

    # ─── Inject template folder into app Jinja2 loader ────────────────────────
    try:
        from jinja2 import FileSystemLoader, ChoiceLoader
        _drive_tpl_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "templates")
        if os.path.isdir(_drive_tpl_dir):
            current_loader = app.jinja_loader
            if current_loader is None:
                app.jinja_loader = FileSystemLoader(_drive_tpl_dir)
            else:
                existing_dirs = []
                if hasattr(current_loader, 'searchpath'):
                    existing_dirs = list(current_loader.searchpath)
                elif hasattr(current_loader, 'loaders'):
                    for ldr in current_loader.loaders:
                        if hasattr(ldr, 'searchpath'):
                            existing_dirs.extend(ldr.searchpath)
                if _drive_tpl_dir not in existing_dirs:
                    app.jinja_loader = ChoiceLoader([current_loader, FileSystemLoader(_drive_tpl_dir)])
            _log.info(f"[Drive] Template folder added to Jinja2 loader: {_drive_tpl_dir}")
    except Exception as _tpl_e:
        _log.warning(f"[Drive] Could not inject template folder into Jinja2: {_tpl_e}")

def init_plugin_routes(app, cfg_mgr, hecos_root, log):
    """Standard HPM entrypoint for dynamic API routes registration."""
    init_drive_routes(app, log)
...
@login_required
def drive_page():
    """Renders the Hecos Drive HTML page."""
...
def drive_quick_links():
    ...
    # We use the Hecos project root for scanning system files.
    # routes.py is in hecos/plugins/drive/ - we need to go up 3 levels.
