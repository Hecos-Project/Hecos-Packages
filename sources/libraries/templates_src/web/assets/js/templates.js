...
(function () {
  ...
  const TM = {
    ...
    onChannelChange(ch, tpl) {
      ...
      if (ch === 'email') {
        this._initGrapes(tpl ? (tpl.body_html || '') : '');
      } else {
        const body = tpl ? (tpl.body || '') : '';
        // find the active textarea
        const ta = document.querySelector('#tpl-editor-' + ch + ' .tpl-body-textarea');
        if (ta) { ta.value = body; tplUpdateCharCount(ta); }
      }
    },

    _initGrapes(html) {
      if (_grapes) { _grapes.setComponents(html); return; }
    ...
    async previewRender() {
      ...
      _text('tpl-preview-subject', res.subject || '—');
      const ch = _val('tpl-edit-channel');
      if (ch === 'email' && res.body_html) {
  ...
  window.tplUpdateCharCount = function(ta) {
    const countEl = ta.closest('.tpl-textarea-wrap')?.querySelector('.tpl-char-count span');
