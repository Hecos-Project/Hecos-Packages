...
Endpoints:
    ...
    POST /api/mail/sync-all                  sync all folders from IMAP
...
def _get_account_id() -> str:
    ...
    aid = request.headers.get("X-Mail-Account") or request.args.get("account_id")
    if not aid:
        cfg = _get_cfg()
        aid = cfg.get("active_account_id", "default")
    return aid
...
def list_messages():
    ...
    try:
        if query:
            items = store.search_messages(account_id=aid, query=query, folder=folder if folder != "ALL" else None, limit=limit)
        else:
            items = store.list_folder(account_id=aid, folder=folder, limit=limit,
...
def send_email():
    ...
    try:
        ...
        client = build_smtp_client(cfg, username, account_id=aid)
        ok, msg = client.send(
...
def sync_all():
    ...
    try:
        ...
        cfg      = _get_cfg()
        username = _get_username()
        client = build_imap_client(cfg, username, account_id=aid)
        ...
        for folder in folders:
            try:
                known    = store.get_uid_set(aid, folder)
...
def get_stats():
    ...
    aid = _get_account_id()
    try:
        return jsonify({"ok": True, "stats": store.get_stats(aid)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def list_drafts():
    from hecos.hpm.mail import store
    aid = _get_account_id()
    return jsonify({"ok": True, "drafts": store.list_drafts(aid)})
...
def create_draft():
    ...
    from hecos.hpm.mail import store
    data = request.get_json(force=True) or {}
    d = store.save_draft(account_id=aid,
...
def test_account():
    ...
    try:
        ...
        cfg      = _get_cfg()
        username = _get_username()
        smtp_ok, smtp_msg = build_smtp_client(cfg, username, account_id=aid).test_connection()
        imap_ok, imap_msg = build_imap_client(cfg, username, account_id=aid).test_connection()
        return jsonify({
...
def get_account_info():
    """Returns the mail address currently configured (never returns the password)."""
    try:
...
def get_mail_config():
    """Returns the current MAIL plugin settings (excludes raw credentials)."""
    try:
        cfg  = _get_cfg()
...
def update_mail_config():
    ...
    SCALAR_KEYS = {
        ...
        "max_messages", "active_account_id",
    }
    try:
        ...
        # Handle accounts list — replaces entire list atomically
        if "accounts" in data:
            accounts = data["accounts"]
            if isinstance(accounts, list):
                current["accounts"] = accounts
                # Keep active_account_id pointing to a valid account
                ids = [a.get("id") for a in accounts if a.get("id")]
                if ids and current.get("active_account_id") not in ids:
                    current["active_account_id"] = ids[0]

        ok = save_config(current)
        logger.info(f"[MAIL] Config saved: {len(current.get('accounts', []))} accounts")
        return jsonify({"ok": ok})
    except Exception as e:
