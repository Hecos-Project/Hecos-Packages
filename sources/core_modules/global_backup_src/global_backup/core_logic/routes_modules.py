...
def register_module_backup_routes(app) -> None:
    ...
    def system_config_backup():
        """Esporta tutti i file YAML/JSON/TOML da config/data."""
        try:
            import os
            ...
            if os.path.isdir(data_dir):
                ...
                for fname in os.listdir(data_dir):
                    if fname.endswith((".yaml", ".json", ".toml")):
    ...
    def system_config_restore():
        ...
        try:
            ...
            for fname, content in files_data.items():
                if not fname.endswith((".yaml", ".json", ".toml")):
                    continue
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # HPM CONFIGS (*.toml files inside package dirs)
    # ═══════════════════════════════════════════════════════════════════════════

    @app.route("/hecos/api/backup_module/hpm_configs/backup", methods=["GET"], endpoint="mbkp_hpm_configs_backup")
    def hpm_configs_backup():
        """Esporta tutti i TOML di configurazione dei package HPM installati."""
        try:
            import os
            root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
            ...
            if os.path.isdir(hpm_dir):
                for pkg in os.listdir(hpm_dir):
                    pkg_path = os.path.join(hpm_dir, pkg)
                    if not os.path.isdir(pkg_path):
                        continue
                    
                    # Cerca in sottocartelle del package config come "<pkg>_config/*.toml"
                    for walk_root, dirs, files in os.walk(pkg_path):
                        # Evita cartelle inutili
                        dirs[:] = [d for d in dirs if d not in ('__pycache__', 'web_ui', 'static', 'templates', 'node_modules')]
                        for f in files:
                            if f.endswith('.toml') and f != 'hpkg_manifest.toml':
                                rel_path = os.path.relpath(os.path.join(walk_root, f), hpm_dir)
                                fpath = os.path.join(walk_root, f)
                                with open(fpath, "r", encoding="utf-8") as file_handle:
                                    all_files[rel_path] = file_handle.read()
            
            return jsonify({
                "ok": True, 
                "_is_files_bundle": True,
                "files": {
                    f"hpm_configs/{path.replace(chr(92), '/')}": content for path, content in all_files.items()
                }
            })
        except Exception as e:
            logger.error(f"[BACKUP] hpm_configs_backup error: {e}")
            return jsonify({"ok": False, "error": str(e)}), 500

    @app.route("/hecos/api/backup_module/hpm_configs/restore", methods=["POST"], endpoint="mbkp_hpm_configs_restore")
    def hpm_configs_restore():
        """Ripristina i file TOML di configurazione HPM."""
        try:
            import os
            body = request.get_json(force=True) or {}
            files_data = body.get("data", {})
            ...
            if not isinstance(files_data, dict):
                return jsonify({"ok": False, "error": "data must be a dict"}), 400
            ...
            for rel_path, content in files_data.items():
                if ".." in rel_path or not rel_path.endswith('.toml'):
                    continue
                
                fpath = os.path.join(hpm_dir, os.path.normpath(rel_path))
                ...
                try:
                    ...
                    logger.error(f"[BACKUP] hpm_configs write error '{rel_path}': {fe}")

            return jsonify({"ok": True, "imported": imported})
        except Exception as e:
            logger.error(f"[BACKUP] hpm_configs_restore error: {e}")
            return jsonify({"ok": False, "error": str(e)}), 500
