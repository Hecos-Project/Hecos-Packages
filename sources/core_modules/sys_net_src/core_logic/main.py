...
def is_admin():
    """Returns True if Hecos is running with Windows Administrator privileges."""
    try:
...
class SysNetTools:
    """
    Plugin: Sys_Net
    Advanced networking tools for Hecos. Allows the AI to map the local network,
    ...
    def get_proxy_status(self) -> str:
        """
        Reports the current proxy configuration status in Hecos, including the URL and whether it is active.
        ...
        if not proxy_url:
            ...
            try:
                ...
                ip = "(non disponibile)"
            return (
                f"**Stato Proxy Hecos: DISATTIVO** ❌\n"
                f"Nessun proxy configurato in Hecos. Le richieste usano la connessione diretta.\n"
    ...
    def set_dns_servers(self, primary: str, secondary: str) -> str:
        ...
        Requires Hecos to be running as Administrator!
        ...
        if not is_admin():
            ...
            return "PERMISSION DENIED: Modifying DNS requires Hecos to be running as Windows Administrator. Please restart Hecos as Admin."
    ...
    def reset_dns(self) -> str:
        ...
        if not is_admin():
            ...
            return "PERMISSION DENIED: Resetting DNS requires Hecos to be running as Windows Administrator."
